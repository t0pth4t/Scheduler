...
#include <fstream>
	int main(){
		...
		char cinput[512];
		string input;
		cout << "Scheduler --help for command options\n";
		do{
			cout << '$';
			cin.getline (cinput,512);
			input = cinput;
			size_t size = countWords(input);
			if(size != 0){
			string parsed[size];
			...
			string cmd = parsed[0];
				...
				if(cmd =="add") add(schedule, parsed, size);
					...
					if(size == 2)exportSch(schedule, parsed[1]);
				}
				else if(cmd =="import"){
					if(size ==2) importSch(schedule,parsed[1]);
				}
				else if(cmd =="remove"){
